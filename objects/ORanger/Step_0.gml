...
if (abs(target.x - x) <= 100 and abs(target.y - y) <= 40 and state != RSTATE.HIT and state != RSTATE.DEAD and on_ground()) {
    hsp = 0;
    state = RSTATE.ATK;
} else if ((abs(target.x - x) > 100 or abs(target.y - y) > 40) and state == RSTATE.ATK) {
    state = RSTATE.IDLE;
}
